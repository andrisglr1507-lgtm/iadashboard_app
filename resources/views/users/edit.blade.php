    ...
    <form action="{{ route('users.update', $user->id) }}" method="POST">
        ...
        <div style="margin-bottom: 16px;">
            ...
            <select name="role" required style="width: 100%; padding: 10px 14px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 0.9rem; background: white;">
                ...
                <option value="HEAD_OPNAME" {{ old('role', $user->role) == 'HEAD_OPNAME' ? 'selected' : '' }}>HEAD_OPNAME</option>
                <option value="AUDITOR" {{ old('role', $user->role) == 'AUDITOR' ? 'selected' : '' }}>AUDITOR</option>
            </select>
        </div>

        <div style="margin-bottom: 16px;">
            <label style="display: block; font-weight: 600; margin-bottom: 8px; font-size: 0.9rem;">Password <span style="color:#94a3b8; font-size: 0.8rem; font-weight: normal;">(Leave blank to keep current)</span></label>
            <input type="password" name="password" minlength="6"
                   style="width: 100%; padding: 10px 14px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 0.9rem;" placeholder="Enter new password to change">
        </div>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 12px;">
            ...
            <button type="submit" class="btn-primary" style="padding: 10px 20px; font-size: 0.9rem;">Update User</button>
        </div>
    </form>
</div>
@endsection
