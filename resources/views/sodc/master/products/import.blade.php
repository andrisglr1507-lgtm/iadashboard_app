...
@section('page_title', 'Import Master Product')
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 1px solid #e2e8f0;">
        <div>
            <h3 style="margin-top: 0; color: #1e293b; display: flex; align-items: center; gap: 10px;">
                <i class="fas fa-box-open" style="color: #107c41; font-size: 1.5rem;"></i> Upload File CSV Product
            </h3>
            <p style="color: #64748b; font-size: 0.95rem; margin: 0;">Silakan upload file master Product Anda. Jika SKU sudah ada, data akan di-update (<i>Update or Create</i>).</p>
            <p style="color: #475569; font-size: 0.85rem; margin: 10px 0 0 0; background: #f8fafc; padding: 10px; border-radius: 6px; border: 1px dashed #cbd5e1;">
                ...
                <code style="color: #e21d48;">sku_code, sku_name, barcode, packname, uom, principal</code>
            </p>
        </div>
        <a href="{{ route('sodc.products.template') }}" style="display: inline-flex; align-items: center; gap: 8px; background: #f1f5f9; color: #0f172a; border: 1px solid #cbd5e1; padding: 8px 16px; border-radius: 8px; font-size: 0.85rem; font-weight: 600; text-decoration: none;">
            <i class="fas fa-download"></i> Download Template CSV
        </a>
    </div>

    <form action="{{ route('sodc.products.import') }}" method="POST" enctype="multipart/form-data">
        @csrf
