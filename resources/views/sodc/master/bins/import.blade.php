@extends('layouts.app')
@section('title', 'Import Bin')
@section('page_title', 'Import Master Bin')
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 1px solid #e2e8f0;">
        <div>
            <h3 style="margin-top: 0; color: #1e293b; display: flex; align-items: center; gap: 10px;">
                <i class="fas fa-file-csv" style="color: #107c41; font-size: 1.5rem;"></i> Upload File CSV
            </h3>
            <p style="color: #64748b; font-size: 0.95rem; margin: 0;">Silakan upload file master Bin Anda. Sistem akan secara otomatis mendeteksi Zona, Level, dan memetakan Ganjil/Genap.</p>
        </div>
        <a href="{{ route('sodc.bins.template') }}" style="display: inline-flex; align-items: center; gap: 8px; background: #f1f5f9; color: #0f172a; border: 1px solid #cbd5e1; padding: 8px 16px; border-radius: 8px; font-size: 0.85rem; font-weight: 600; text-decoration: none;">
            <i class="fas fa-download"></i> Download Template CSV
        </a>
    </div>

    <form action="{{ route('sodc.bins.import') }}" method="POST" enctype="multipart/form-data">
        @csrf
