            ...
            <tbody>
                @foreach ($summaryData as $row)
                    @php
                        ...
                        $isTypoRow = ($row['stock_system'] == 0 && $hasInputS);
                    @endphp
                    <tr class="{{ $isFinalRow ? 'row-final' : '' }} {{ $isTypoRow ? 'row-typo-highlight' : '' }}"
                        ...
                        <!-- Team S -->
                        <td class="text-right num-col">{{ $row['s_karton'] !== '-' ? number_format($row['s_karton']) : '-' }}</td>
                        <td class="text-right num-col">{{ $row['s_pcs'] !== '-' ? number_format($row['s_pcs']) : '-' }}</td>
                        <td class="text-right num-col font-semibold {{ $isTypoRow ? 'text-orange-medium' : '' }}">
                            {{ $row['s_total'] !== '-' ? number_format($row['s_total']) : '-' }}
                        </td>
                        ...
                        <!-- R3 -->
                        <td class="text-right num-col">{{ $row['r3_karton'] !== '-' ? number_format($row['r3_karton']) : '-' }}</td>
                        <td class="text-right num-col">{{ $row['r3_pcs'] !== '-' ? number_format($row['r3_pcs']) : '-' }}</td>
                        <td class="text-right num-col font-semibold">{{ $row['r3_total'] !== '-' ? number_format($row['r3_total']) : '-' }}</td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        async function loadModalHistory(productId) {
            ...
            try {
                const response = await fetch('{{ route("single_mode.recount.detail") }}', {
                    method: 'POST',
                    headers: {
        ...
        $('#modalTable').on('click', '.btn-save-row', async function() {
            ...
            try {
                const response = await fetch('{{ route("single_mode.recount.edit") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $('#modalTable').on('click', '.btn-delete-row', async function() {
            ...
            try {
                const response = await fetch('{{ route("single_mode.recount.delete") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $('#assignBtn').on('click', async function() {
            ...
            try {
                const response = await fetch('{{ route("single_mode.recount.assign") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
