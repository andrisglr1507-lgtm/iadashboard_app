...
class RecountSingleController extends Controller
{
    /**
     * Main recount page
     */
    public function index()
    {
        ...
        if (!$activeSessionId) {
            ...
            return view('single_mode.recount', compact('error', 'summaryData', 'comprehensiveAnalysis', 'stats', 'principals', 'users', 'activeSessionId'));
        }

        try {
            // Validate session is mode 'S' or 'single'
            ...
            // Get count data (S, R1, R2, R3) grouped by product and team
            $countResult = OpnameCountHeader::where('opname_count_headers.session_id', $activeSessionId)
                ...
                ->whereIn('opname_count_headers.team', ['S', 'R1', 'R2', 'R3'])
                ->groupBy('d.id_product', 'opname_count_headers.team')
                ->select(
                    'd.id_product', 'opname_count_headers.team',
            ...
            foreach ($countResult as $row) {
                $countData[$row->id_product][$row->team] = [
            ...
            $activeAssignmentsRaw = OpnameRecountAssignment::where('session_id', $activeSessionId)
                ...
                ->select('id_product', 'round_number', 'status')
                ->get();
            
            $activeAssignments = [];
            foreach ($activeAssignmentsRaw as $assignment) {
                $activeAssignments[$assignment->id_product][$assignment->round_number] = true;
            }
            ...
            foreach ($stockSystemData as $productId => $stockSystem) {
                ...
                $result = $this->calculateSingleResult($teamS, $stockSystem, $teamR1, $teamR2, $teamR3, $productId, $activeAssignments);
                ...
                if ($stockSystem == 0 && $hasInputS) {
                    ...
                    if (!empty($similarWithStock)) {
                        $analisis = [
                            'has_suggestion' => true,
                            'message' => '⚠️ Stok sistem 0, tim opname menghitung ' . number_format($teamS['final_qty']) . ' unit. Kemungkinan kesalahan kode!',
                        ...
                        $analisis = [
                            'has_suggestion' => true,
                            'message' => 'ℹ️ Stok sistem 0, ada input Team S (' . number_format($teamS['final_qty']) . ' unit). Ditemukan kode mirip (stok 0 juga)',
                            'suggestions' => $similar,
                            'type' => 'similar_code',
                            's_qty' => $teamS['final_qty'],
                        ];
                        ...
                        $analisis = [
                            'has_suggestion' => true,
                            'message' => '⚠️ ANOMALI: Stok 0, tim opname menghitung ' . number_format($teamS['final_qty']) . ' unit. Perlu investigasi fisik!',
                            'suggestions' => [],
                            'type' => 'anomaly',
                            's_qty' => $teamS['final_qty'],
                        ];
                ...
                // Calculate stock_system karton and pcs based on UOM
                $uomVal = (int)$uom;
    ...
    public function assignUsers(Request $request)
    {
        ...
        foreach ($selectedProducts as $product) {
            ...
            // Find all unique locations for this product
            $locations = OpnameCountHeader::where('opname_count_headers.session_id', $sessionId)
                ->join('opname_count_details as d', 'opname_count_headers.count_id', '=', 'd.count_id')
                ->where('d.id_product', $productId)
                ->distinct()->pluck('opname_count_headers.location_code')->toArray();

            if (empty($locations)) {
                $sess = OpnameSession::where('session_id', $sessionId)->first();
    ...
    private function calculateSingleResult($teamS, $stockSystem, $teamR1, $teamR2, $teamR3, $productId, $activeAssignments)
    {
        ...
        if ($r3Total !== null) return ['stage' => 'final', 'label' => 'FINAL R3', 'final_total' => $r3Total, 'can_assign' => false, 'round_to_assign' => null];
        if ($r2Total !== null) return ['stage' => 'final', 'label' => 'FINAL R2', 'final_total' => $r2Total, 'can_assign' => false, 'round_to_assign' => null];
        if ($sTotal !== null && $sTotal == $stockSystem) return ['stage' => 'final', 'label' => 'MATCH', 'final_total' => $sTotal, 'can_assign' => false, 'round_to_assign' => null];
        if ($r1Total !== null) {
            if ($r1Total == $sTotal || $r1Total == $stockSystem) return ['stage' => 'final', 'label' => 'FINAL R1', 'final_total' => $r1Total, 'can_assign' => false, 'round_to_assign' => null];
            
            if (isset($activeAssignments[$productId][3])) {
        ...
        if ($sTotal !== null && $sTotal != $stockSystem) {
            if (isset($activeAssignments[$productId][2])) {
        ...
        foreach ($stockSystemData as $productId => $stockSystem) {
            ...
            if (!empty($matches)) {
                ...
                $analysis[] = [
                    ...
                    'stock_system' => $stockSystem, 's_qty' => $teamS['final_qty'],
                    's_karton' => $teamS['karton'], 's_pcs' => $teamS['pcs'],
